...
spawn origin, lifecycle ownership, and consumer-held Fork control are
...
- **Residency** is the Runtime and its Registry keeping each admitted
  ...
  Fork changes neither residency nor lifecycle.
...
- **Lifecycle ownership** ends at the generation boundary. Ordinary
  ...
  another Fiber.
- **Fork control** is explicit consumer control over one non-root Fiber.
  Consumers compose delivered Forks themselves: the Harness Roster
  records Forks in spawn order and disposes them in reverse spawn order,
...
duration. Dropping Context or Fork handles never initiates root or
...
Generation ownership plus Registry residency plus consumer Fork
...
before its Fork has been delivered: such a drain would require a core
...
| No structured core owner; Harness retains returned Forks | preserves an existing ADR | ADRs 0005 and 0011 |

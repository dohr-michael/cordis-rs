...
> **正在转入 Legacy。** `0.6.x` 版本线将在项目迁移到 v3 运行时架构期间进入维护模式。`0.6.x` 不再规划新功能，只保留关键 bug 修复和安全修复。v3 将先在独立分支上继续开发，在完成发布准备后再成为默认开发线。

现有用户仍可继续使用 `0.6.x` API；新的开发应在 v3 发布后以 v3 为目标。项目仍处于 1.0 之前，v3 迁移会被明确视为一次破坏性的架构边界。

`0.6.x` crate 已移植完整的**核心运行时**：
